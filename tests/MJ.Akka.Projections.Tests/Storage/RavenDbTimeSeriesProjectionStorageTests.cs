        ...
        SetupRavenDbStorage>, IClassFixture<RavenDbFixture>
{
    ...
    protected override async Task VerifyContext(RavenDbProjectionContext<TestDocument> loaded)
    {
        ...
        timeSeriesValues.Length.ShouldBe(1);

        timeSeriesValues[0].Value.ShouldBe(5d);
        timeSeriesValues[0].Tag.ShouldBe("tag");
    }
    ...
    private class TestProjection : RavenDbProjection<TestDocument>
    {
        ...
        public override Task<IProjectionEventSource> GetSource() =>
            Task.FromResult<IProjectionEventSource>(new SimpleProjectionEventSource(
                (_, _) => Source.From(ImmutableList<EventWithPosition>.Empty)));
    }
}